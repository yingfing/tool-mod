...
3. **幽灵法杖槽位**：在配置菜单中确保 `enable_phantom_slot` 开启（默认开），并在「幽灵法杖槽位」分类下查看/修改热键。
   ...
   - 切换方式按优先级自动选择，**全部是服务端认可的真实数据包**：
     1. 快捷栏其他格有法杖 → 发 `ServerboundSetCarriedItemPacket` 切选中格
     2. 主背包里有法杖 → 发 `ServerboundContainerClickPacket(SWAP)` 交换到主手
...
- 生存模式下交换依赖 `ServerboundContainerClickPacket`，若服务端反作弊对容器点击包做严格校验，可能失败；此时建议把法杖放在**快捷栏**（走 `SetCarriedItem`，最稳）。
